...
- OpenAI-compatible Chat Completions defaults to no server-side tools. Set `OPENAI_COMPATIBLE_TOOLS=web_search` or `web_search,x_search` only for compatible relays that support those tools. Do not use `web_search_preview` or legacy `search_parameters`. These tools do not fetch result pages automatically; use `fetch` when page text is required.
...
- For the current xAI Responses model set, prefer `grok-4.20-multi-agent-console` as the primary search model. Configure `XAI_MODEL_FALLBACKS=grok-4.20-multi-agent-xhigh,grok-4.20-multi-agent-high` as same-provider fallbacks. Keep `grok-4.20-0309-reasoning-console` out of the automatic chain unless a live retest proves it can complete web/search calls reliably.
- Do not promote OpenRouter Grok `:online` / native web search into the default main-search chain only because model-card latency looks good. Live checks showed ordinary OpenRouter inference can be fast while native/online web search is much slower and more expensive. Keep OpenRouter as an optional peer `main_search` fallback only; if it is used for search, prefer the OpenRouter web plugin with `engine=exa` over native/`:online`.
...
- `OPENAI_COMPATIBLE_TOOLS` defaults to empty and accepts only `web_search` and `x_search`; configured tools are sent only on OpenAI-compatible `search()`.
